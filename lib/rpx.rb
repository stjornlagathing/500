...
module Rpx
  ...
  class RpxHelper
    ...
    def auth_info(token)
      ...
      resp = data['profile'].map { |k,v| "#{CGI::escape k.to_s}=#{CGI::escape v.to_s}"}.join(' ')
      Rails.logger.info "profile: " +  resp.to_s
      data['profile']
    end
    ...
    def api_call(method_name, partial_query)
      ...
      if data['stat'] != 'ok'
        msg = data.map { |k,v| "#{CGI::escape k.to_s}=#{CGI::escape v.to_s}" }.join(' ')
        Rails.logger.error "error: " +  msg.to_s
        raise RpxException.new(resp), 'Unexpected API error'
      end
      resp = data.map { |k,v| "#{CGI::escape k.to_s}=#{CGI::escape v.to_s}"}.join(' ')
      Rails.logger.info "ok: " +  resp.to_s
      data
    end
